...
MRichTextEdit::MRichTextEdit(QWidget *parent) : QWidget(parent) {
    ...
    QAction *removeFormat = new QAction(QStringLiteral("删除所有文本格式"), this);
    ...
    QAction *removeAllFormat = new QAction(QStringLiteral("删除所有格式"), this);
    connect(removeAllFormat, SIGNAL(triggered()), this, SLOT(textRemoveAllFormat()));
    f_textedit->addAction(removeAllFormat);

    QAction *textsource = new QAction(QStringLiteral("查看html源码"), this);
...
void MRichTextEdit::setPlainText(const QString &text) { f_textedit->setPlainText(text); }

void MRichTextEdit::setHtml(const QString &text) { f_textedit->setHtml(text); }
